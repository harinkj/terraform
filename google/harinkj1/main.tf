provider "google" {
  alias = "current"
//  description = "harinkj"
  credentials = "${file("../accounts/GoogleSigninSample-d19a9a1b23f5.json")}"
  project     = "signinsample-1"
  region      = "us-east1"
}
provider "google" {
  alias = "harinkj"
...
resource "google_compute_instance_template" "instance-template" {
  ...
  metadata {
    startup-script = "echo \" export PROFILE=DEV\" > /etc/init.d/test.sh;chmod 755 /etc/init.d/test.sh"
  }
